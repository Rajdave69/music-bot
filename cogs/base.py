...
class Queue:
    ...
    def get_next_song(self, guild_id: int, override=False) -> list or None:
...
class DedicatedButtons(discord.ui.View):
    ...
    # == Row 1 ==
    @discord.ui.button(emoji="<:music_loop:1003968639189397574>", style=discord.ButtonStyle.green,
                       custom_id="music_loop", row=1)
    ...
    @discord.ui.button(emoji="<:music_pause:1003968631731925082>", style=discord.ButtonStyle.blurple,
                       custom_id="music_playpause", row=1)
    ...
    @discord.ui.button(emoji="<:music_skip:1003968637054484610>", style=discord.ButtonStyle.green,
                       custom_id="music_skip", row=1)
    ...
    @discord.ui.button(emoji="<:music_stop:1003968634311426091>", style=discord.ButtonStyle.red, custom_id="music_stop",
                       row=1)
    ...
    # == Row 2 ==
    @discord.ui.button(emoji="<:music_vol_down:1003968641064255521>", style=discord.ButtonStyle.green,
                       custom_id="music_vol_down", row=2)
    ...
    @discord.ui.button(emoji="<:shuffle:1004287992984240148>", style=discord.ButtonStyle.blurple,
                       custom_id="music_shuffle", row=2)
    ...
    @discord.ui.button(emoji="<:music_vol_up:1003968645820596287>", style=discord.ButtonStyle.green,
                       custom_id="music_vol_up", row=2)
    ...
    @discord.ui.button(emoji="<:music_heart:1003968643656319047>", style=discord.ButtonStyle.red,
                       custom_id="music_heart", row=2)
    async def heart_button(self, button, interaction):
        db = sqlite3.connect("./data/music.db")
        c = db.cursor()
        c.execute(
            f"CREATE TABLE IF NOT EXISTS playlists_{interaction.user.id} (playlist TEXT, song_id TEXT, titles TEXT)")
        db.commit()
        ...
        c.execute(
            f'SELECT song_id FROM playlists_{interaction.user.id} WHERE song_id="{self.queues.current_song[0]}" AND playlist="main"')
        res = c.fetchone()
        if res is not None:
        ...
        c.execute(
            f'INSERT INTO playlists_{interaction.user.id} (playlist, song_id, titles) values("main", "{self.queues.current_song[0]}", "{self.queues.current_song[1][0]}")')
        db.commit()

        p_embed = discord.Embed(title="Music | Playlist", color=embed_color, url=embed_url,
                                description=f"Adding to the playlist was successful!")
...
class Music(commands.Cog):
    ...
    async def m_setup(self, ctx):
        ...
        m_embed.set_image(
            url="https://media.discordapp.net/attachments/988082459658813490/1004316068476624977/music_banner.jpg")
    ...
    async def queuelist(self, ctx):
        ...
        queue_list = [f"{self.queues.song_list[str(ctx.guild.id)][i][1]}" for i in
                      range(len(self.queues.song_list[str(ctx.guild.id)]))][::-1]
    ...
    async def add(self, ctx, playlist_name: str):
        ...
        self.c.execute(
            f"CREATE TABLE IF NOT EXISTS playlists_{ctx.author.id} (playlist TEXT, song_id TEXT, titles TEXT)")
        self.db.commit()
        ...
        self.c.execute(
            f'SELECT song_id FROM playlists_{ctx.author.id} WHERE song_id="{self.queues.current_song[0]}" AND playlist="{playlist_name}"')
        res = self.c.fetchone()
        if res is not None:
            log.debug(res)
        ...
        self.c.execute(
            f'INSERT INTO playlists_{ctx.author.id} (playlist, song_id, titles) values("{playlist_name}", "{self.queues.current_song[0]}", "{self.queues.current_song[1][0]}")')
        self.db.commit()
    ...
    async def show_playlists(self, ctx):
        ...
        m_embed.set_footer(text=embed_footer)
        m_embed.set_author(name=embed_header, icon_url=embed_icon)
        m_embed.add_field(name="Playlists", value="\n".join([f"`{playlist[0]}`" for playlist in playlists]),
                          inline=False)
        await ctx.respond(embed=m_embed)
    ...
    async def removesong(self, ctx, playlist_name: str):
        playlist_name = await input_sanitizer(playlist_name)
        self.c.execute(
            f'DELETE FROM playlists_{ctx.author.id} WHERE song_id="{self.queues.current_song[0]}" AND playlist="{playlist_name}"')
        self.db.commit()

        p_embed = discord.Embed(title="Music | Playlist", color=embed_color, url=embed_url,
                                description=f"Deleting the song from the playlist was successful!")
        p_embed.add_field(name="Song", value=f"{self.queues.current_song[1]}", inline=False)
        p_embed.add_field(name="Playlist", value=f"`{playlist_name}`", inline=False)
        p_embed.set_footer(text=embed_footer)
        p_embed.set_author(name=embed_header, icon_url=embed_icon)
    ...
    async def rename_playlist(self, ctx, old_name: str, new_name: str):
        ...
        p_embed = discord.Embed(title="Music | Playlist", color=embed_color, url=embed_url,
                                description=f"Renaming the playlist was successful!")
    ...
    async def deleteall(self, ctx, playlist_name: str):
        ...
        msg = await ctx.respond(
            f"You have {songs} in the {playlist_name} playlist! Are you sure you want to delete all songs from this playlist?",
            view=_view)
        ...
        p_embed = discord.Embed(title="Music | Playlist", color=embed_color, url=embed_url,
                                description=f"Deleting the playlist was successful!")
        p_embed.add_field(name="Playlist", value=f"`{playlist_name}`", inline=False)
